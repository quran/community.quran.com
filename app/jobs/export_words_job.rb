class ExportWordsJob < ApplicationJob
  ...
  def prepare_db(file_path)
    ...
    ExportRecord.connection.execute "CREATE TABLE words(sura integer,ayah integer, word_position integer, translation text, transliteration text, text_madani text, glyph text, word_type text, audio_url text, primary key(sura, ayah, word_position))"
    ExportRecord.table_name = 'words'
  end
  
  def prepare_import_sql()
    Word.includes(:transliterations, :en_translation).find_each do |word|
      translation    = ExportRecord.connection.quote(word.en_translation&.text)
      ...
      value          = "(#{chapter}, #{verse}, #{word.position}, #{translation}, #{translitration}, #{text}, #{code}, #{w_type}, #{audio})"
      begin
        ExportRecord.connection.execute "INSERT INTO words (sura, ayah, word_position, translation, transliteration,text_madani, glyph, word_type, audio_url) VALUES #{value}"
      rescue Exception => e
      end
    end
  end
