module Utils
  class CyrillicToLatin
    UZ_ALPHABET = {
        ...
        'Ш': 'Sh', 'Ч': 'Ch', 'Ё': 'Yo', 'Ю': 'Yu', 'Я': 'Ya', 'Йe': 'Ye', 'ъ': `'`,
    }

    UZ_VOWELS = ['А', 'а', 'Э', 'э', 'И', 'и', 'О', 'о', 'У', 'у', 'Ў', 'ў']
    ...
    def convert_chars(cyril, i, array)
      if (UZ_ALPHABET[cyril.to_sym])
        if (cyril === 'Е' || cyril === 'е')
          cyril = handle_ye(cyril, i, array)
        end

        if (cyril === 'Ц' || cyril === 'ц')
          cyril = handle_ts(cyril, i, array)
        end
    ...
    def handle_ts(cyril, index, array)
      after_vowel = index > 0 && UZ_VOWELS.include?(array[index - 1])
